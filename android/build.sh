...
# Native capabilities from installed packages: a package declares its Kotlin in its own
# chuks.json (see appconfig.chuks), the sources compile in beside the host, and a
# generated registry routes the commands whose namespace they claim. The iOS half of this
# lives in ios/build.sh; the two must stay in step, since a package ships both.
...
KT_SRC="$PKGDIR/MainActivity.kt $PKGDIR/ChuksEffects.kt $PKGDIR/ChuksModule.kt $PKGDIR/ChuksJobService.kt $OUT/ChuksPackageModules.kt $PKG_KT"; KT_CP="$AJAR"; ZXING="$PKGDIR/libs/zxing-core.jar"
if [ "$PREVIEW" = "1" ]; then
